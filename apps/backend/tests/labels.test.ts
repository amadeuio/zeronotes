...
describe('Labels Endpoints', () => {
  ...
  describe('POST /api/labels', () => {
    ...
    it('should return 400 when id is missing', async () => {
      const response = await api.post('/api/labels').set('Authorization', `Bearer ${token}`).send({
        name: 'Test Label',
      });
  ...
  describe('GET /api/labels', () => {
    ...
    it('should not return labels from other users', async () => {
      ...
      const response = await api.get('/api/labels').set('Authorization', `Bearer ${token2}`);
  ...
  describe('DELETE /api/labels/:id', () => {
    it('should delete a label', async () => {
      ...
      // Verify it's deleted
      const getResponse = await api.get('/api/labels').set('Authorization', `Bearer ${token}`);
