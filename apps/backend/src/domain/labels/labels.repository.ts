...
export const labelRepository = {
  findAll: async (userId: string): Promise<LabelRow[]> => {
  ...
  create: async (userId: string, id: string, name: string): Promise<LabelRow> => {
    const result = await pool.query(
  ...
  update: async (userId: string, id: string, name: string): Promise<LabelRow> => {
    const result = await pool.query(
  ...
  delete: async (userId: string, id: string): Promise<boolean> => {
    const result = await pool.query('DELETE FROM labels WHERE id = $1 AND user_id = $2', [
      id,
      userId,
    ]);
